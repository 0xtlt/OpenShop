---
title: Testing
description: Test context options, provider fakes, signed requests, factories, and cleanup contracts.
---

To set up a test suite, see [Test an app](/guides/test-app/).
...
describe('OpenShop app', () => {
  ...
  beforeEach(async () => {
    ctx = await createTestContext({
      // Run from the app root; configPath defaults to openshop.config.ts.
...
`openshop test` requires an app-owned `tests/bootstrap.ts`. It attempts a schema
push against `DATABASE_URL`, then executes that bootstrap with Node.js. The
minimal generated project has a `test` script but does not include a bootstrap.
Follow [Test an app](/guides/test-app/) for a complete setup.

The context does not start a worker, isolate or truncate the database, or mock
Shopify network requests. `shutdown()` closes the HTTP server and cleans up
factory resources; it does not delete all app-owned database rows.
