...
export default function Home() {
  ...
  const flowsEnabled = pages.flows !== 'disabled'
  const cronsEnabled = pages.crons !== 'disabled'
  const providersEnabled = pages.providers !== 'disabled'
    ...
    const load = async () => {
      ...
      try {
        const [flows, runs, cronItems, providerItems] = await Promise.all([
          flowsEnabled ? apiJson<FlowSummary[]>('/api/flows') : Promise.resolve([]),
          flowsEnabled ? apiJson<FlowRun[]>('/api/runs?limit=10') : Promise.resolve([]),
          cronsEnabled ? apiJson<CronItem[]>('/api/crons') : Promise.resolve([]),
          providersEnabled ? apiJson<ProviderSummary[]>('/api/providers') : Promise.resolve([]),
        ])
  ...
  }, [flowsEnabled, cronsEnabled, providersEnabled])
