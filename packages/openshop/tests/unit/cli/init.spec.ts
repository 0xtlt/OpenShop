...
test.group('init cli', () => {
  test('creates a minimal pnpm project', async ({ assert }) => {
    await withTempDir(async () => {
      ...
      assert.equal(packageJson.scripts['db:generate'], 'openshop migrate generate')
      assert.equal(packageJson.scripts['db:migrate'], 'openshop migrate')
      ...
      assert.exists(readFileSync(resolve(result.targetDir, 'drizzle', 'meta', '_journal.json'), 'utf8'))
